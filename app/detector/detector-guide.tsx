export function DetectorGuide({ mode }: { mode: "camera" | "screen" }) {
  return <>
    ...
    <ol className="detector-steps" aria-label="How to use the detector">
      ...
      <li><strong>3. Review</strong><p>Compare the proof and enter “Pulled by”. Click Approve for a single matching catalog card, or choose the right candidate when there are several. Correct missing details first.</p></li>
    </ol>
    <details className="detector-help">
      ...
      <div className="detector-help-grid">
        ...
        <section><h3>Autographs and AI review</h3><p>Local autograph detection looks for blue pen-like strokes near a readable certification label. Black ink or unclear cards may need review. This is not an authenticity check.</p><p>Optional AI review sends the saved image to OpenAI and may incur API costs. Your local result is saved first. AI suggestions arrive separately and only change the reviewed details when you choose to apply them.</p></section>
        <section><h3>Saving and OBS overlays</h3><p>Captures are uploaded to your review queue. Unsynced frames stay in this browser for retry; do not clear browser storage before syncing. At eight waiting frames, recognition pauses while the live preview stays on. Retry uploads to resume recognition automatically.</p><p>Only confirmed pulls are published. To send them to OBS, copy your session’s overlay key from OBS Studio. A source URL and overlay key are optional.</p><a href="/studio">Open OBS Studio</a></section>
      </div>
    </details>
  </>;
}
