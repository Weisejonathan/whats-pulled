...
export type CachedCatalog = { sets: DetectorSet[]; players: string[]; printRuns?: number[] };
...
export function cachedCatalog(setId = "", storage?: Pick<Storage, "getItem">): CachedCatalog | null {
  try {
    ...
    return { sets: value.sets, players: value.players, ...(Array.isArray(value.printRuns) ? { printRuns: value.printRuns.filter((n: unknown) => typeof n === "number" && Number.isInteger(n) && n > 0 && n <= 99999).slice(0, 1000) } : {}) };
  } catch { return null; }
}
